...
def stackowanie_kanalow_OLI(folder_wejscia,plik_zapisu,x_size,y_size):
    """
    funkcja laczaca ze soba pojedyncze kanaly sensora OLI oraz TIRS w dwa oddzielne pliki
    """
    sterownik=gdal.GetDriverByName('ENVI')
    sterownik.Register()
    zapis=sterownik.Create(plik_zapisu,x_size,y_size,8,gdal.GDT_Int16)#po zmerdzowaniu kodu z innymi trzeba bedzie skorzystac z metody odczytujacej wielkosci rastra z metadanych 
    licznik=0
    for x in utworz_zbiory(znajdz_pliki(folder_wejscia))[1]:
        print x
        raster_band=gdal.Open(folder_wejscia+'\\'+x,gdalconst.GA_ReadOnly)
        zapis.SetProjection(raster_band.GetProjection())
        zapis.SetGeoTransform(raster_band.GetGeoTransform())
        raster_band_data = raster_band.ReadAsArray()
        zapis.GetRasterBand(licznik+1).WriteArray(raster_band_data)
        licznik=licznik+1
    zapis=None
...
def stackowanie_kanalow_TIRS(folder_wejscia,plik_zapisu,x_size,y_size):
    """
    funkcja laczaca ze soba pojedyncze kanaly sensora OLI oraz TIRS w dwa oddzielne pliki
    """
    sterownik=gdal.GetDriverByName('ENVI')
    sterownik.Register()
    zapis=sterownik.Create(plik_zapisu,x_size,y_size,2,gdal.GDT_Int16)#po zmerdzowaniu kodu z innymi trzeba bedzie skorzystac z metody odczytujacej wielkosci rastra z metadanych
    licznik=0
